...
@section('elementos')
    ...
    <form class="col s12" method="post" action="{{ route('updatealumno',$student->id) }}">
        {{ csrf_field() }}
            ...
            <div style="margin-top: 50px">
                ...
                <div class="row">
                    @foreach($degrees as $degree)
                        @if($degree->id === $student->licenciatura)
                    ...
                    <div id="cajalicen" class="input-field col s12 m6 white-text ">
                        <select id="licen" name="licen" required>
                            @foreach($degrees as $degree)
                                <option @if ($student->licenciatura === $degree->id) selected="selected" @endif value="{{ $degree->id }}">{{ $degree->nombre }}</option>
                            @endforeach
                        </select>
        ...
        <div class="row center-align">
            ...
            <a name="cancel" id="cancel" href="{{ route('viewalumno') }}" class="white-text red darken-1 btn boton">Cancelar y volver</a>
        </div>
    </form>
    <script>
