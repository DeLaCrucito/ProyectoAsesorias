...
@section('elementos')
    <form class="col s12" method="post" action="{{ route('updatelicenciatura', $degree->id) }}" >
        {{ csrf_field() }}
                    ...
                    <div class="input-field col s12 m5 white-text">
                        <select id="facultad" name="facultad" required>
                            ...
                            @foreach($facultads as $facultad)
                                <option @if (old('facultad', $degree->facultad) === $facultad->id) selected="selected" @endif value="{{ $facultad->id }}">{{ $facultad->nombre }}</option>
                            @endforeach
                        </select>
                        <label class="white-text">Facultad</label>
                    </div>
                    <div class="input-field col s12 m7">
                        <input class="white-text" type="text" value="{{ $degree->nombre }}" placeholder="Ingrese el nombre de la licenciatura" name="nombre" id="nombre"/>
                        <label class="white-text" for="nombre">Nombre de la licenciatura</label>
                    </div>
                    <div class="input-field col s12 m4">
                        <input class="white-text" type="number" min="1" max="12" value="{{ $degree->semestres }}" placeholder="¿Cuántos semestres tiene la carrera?"
